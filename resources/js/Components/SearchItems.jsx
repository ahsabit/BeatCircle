...
export default function SearchItems({ className , upRef, backFn, ...props}) {
    ...
    const searchRefOne = useRef(null);
    const searchRefTwo = useRef(null);
    const invisibleRef = useRef(null);
    const [historyParams, setHistoryParams] = useState(JSON.parse(localStorage.getItem('queryParams')) || []);

    const searchFn = () => {
        const value = (searchRefOne.current.value || searchRefTwo.current.value);
        if (value) {
            const queryParams = {
                key: value,
                time: Date.now()
            };
            var prevStoredValues = JSON.parse(localStorage.getItem('queryParams'));
    
            if (Array.isArray(prevStoredValues)) {
                prevStoredValues.unshift(queryParams);
    
                while (prevStoredValues.length > 5) {
                    prevStoredValues.pop();
                }
    
                localStorage.setItem('queryParams', JSON.stringify(prevStoredValues));
    
            } else {
                const param = [queryParams];
                localStorage.setItem('queryParams', JSON.stringify(param));
            }
            
            router.get(route('music.search', queryParams));
        }
    };
    ...
    const handleRecommend = () => {
        if (recommendation.current.classList.contains('hidden')) {
            ...
            invisibleRef.current.classList.add('flex');
            invisibleRef.current.classList.remove('hidden');
        }else{
            recommendation.current.classList.remove('flex');
            recommendation.current.classList.add('hidden');
            invisibleRef.current.classList.remove('flex');
            invisibleRef.current.classList.add('hidden');
        }
    };

    const deleteHistory = (index) => {
        var prevStoredValues = JSON.parse(localStorage.getItem('queryParams'));
        prevStoredValues.splice(index, 1);
        localStorage.setItem('queryParams', JSON.stringify(prevStoredValues));
        ...
        <div ref={upRef} id="search_section" className={"relative w-full flex flex-col justify-center items-center " + className} {...props}>
            ...
            <ul ref={recommendation} className='w-full hidden flex-col items-center'>
                ...
                {historyParams.map((param, i) => {
                    return (
                        <li key={i} className="max-w-md w-full bg-base-100 flex">
                            <Link href={route('music.search', param)} className="btn btn-ghost p-4 border border-[#cdcdcd11] flex-1 h-full flex flex-row">
                            ...
                            <svg onClick={() => deleteHistory(i)} className="flex-none m-1 btn btn-ghost h-5 p-4 border border-[#cdcdcd11] flex flex-row" viewBox="0 -0.5 21 21" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink">
            ...
            <div onClick={handleRecommend} ref={invisibleRef} className="bg-[#0000005b] fixed top-0 left-0 right-0 bottom-0 z-40 flex-col hidden">
            </div>
        </div>
    )
}